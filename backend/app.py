...
def recommendation():
    ...
    # Search YouTube for videos related to the title
    query = title.replace(' ', '+')
    search_url = f'https://www.youtube.com/results?search_query={query}'
    response = requests.get(search_url)
    soup = BeautifulSoup(response.text, 'html.parser')

    # Extract video links, titles, and thumbnails
    video_data = []
    for link in soup.find_all('a', class_='yt-uix-tile-link'):
        video_title = link['title']
        video_url = 'https://www.youtube.com' + link['href']

        # Extract video thumbnail URL
        thumbnail_div = link.find_next('div', class_='yt-thumb-simple')
        thumbnail_url = thumbnail_div.find('img')['src']
